...
public class DaoRoom {
	...
	public List<Map<String, Object>> roomSearch(VOChatList pVO) {
		...
		try {
			con = dbMgr.getConnection();
			cstmt = con.prepareCall("{call proc_roomsearch(?,?,?)}");
			int i = 0;
			cstmt.setString(++i, pVO.getMem_id());
			...
			while(rs.next()) {
				rMap = new HashMap<String, Object>();
				rMap.put("clist_code", rs.getString("clist_code").toString());
				rMap.put("chat_nick", rs.getString("chat_nick").toString());
				rMap.put("clist_yourid", rs.getString("clist_yourid").toString());
				rMap.put("clist_img", rs.getString("clist_img").toString());
				rMap.put("clog_contents", rs.getString("clog_contents").toString());
				rMap.put("clog_time", rs.getString("clog_time").toString());
				rList.add(rMap);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void roomChangeName(VOChatNick pVO) {
		StringBuilder sql = new StringBuilder();
		sql.append(" UPDATE chatnick       ");
        sql.append("    SET chat_nick = ?  ");
        sql.append("  WHERE chat_id = ?    ");
        sql.append("    AND clist_code= ?  ");
		try {
			con = dbMgr.getConnection();
			pstmt = con.prepareStatement(sql.toString());
			int i = 0;
			pstmt.setString(++i, pVO.getChat_nick());
			pstmt.setString(++i, pVO.getChat_id());
			pstmt.setString(++i, pVO.getClist_code());
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
