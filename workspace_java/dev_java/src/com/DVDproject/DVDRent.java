...
public class DVDRent implements MouseListener, KeyListener{
	...
	public DVDRent() {
		...
		jmi_msig.addActionListener(new ActionListener() {			
			@Override
			public void actionPerformed(ActionEvent e) {
				System.out.println("회원가입버튼입력");
				insertmemActionPerformed(e);
			}
		});
...
//		jbtn_sel.setForeground(new Color(255,255,255));
	...
	//** 회원관리 이벤트 **//
	protected void insertmemActionPerformed(ActionEvent e) {
		String label = e.getActionCommand();
		memDVD = null;
		memDVD = new MemDVD();
		//입력 할 때는 백지상태에 값을 입력하기 때문에 null값을 입력
		memDVD.set(null, label, this, true);
