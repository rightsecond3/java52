...
//						server.chatList.add(kVO); --> 기존에 존재하는 톡방이기 때문에 추가할 필요 없음.
				...
				case Protocol.ROOM_LIST: {
					...
					break;
				case Protocol.ROOM_CHANGE_NAME: {
					String login_id = stz.nextToken();
					String clist_code = stz.nextToken();
					String change_nick = stz.nextToken();
					VOChatNick pVO = new VOChatNick();
					pVO.setChat_id(login_id);
					pVO.setClist_code(clist_code);
					pVO.setChat_nick(change_nick);
					sCtrl.roomChangeName(pVO);
				}
					break;
				case Protocol.MESSAGE: {
					...
					while(stz.hasMoreTokens()) {
						fileName = stz.nextToken();
						dataSize = stz.nextToken();
					}
