...
public class TalkClient extends JFrame implements ActionListener{
	...
	public void whisper_process() {
		...
		if (index==-1) {//end of file 의미 : 끝까지 찾았는데 없다.
			JOptionPane.showMessageDialog(this, "상대를 선택하세요.");
			return;
		} else {
			String name = (String) dtm_name.getValueAt(index, 0);
			if (nickName.equals(name)) {
				JOptionPane.showMessageDialog(this, "자신 외의 상대를 선택하세요.");
				return;
			} else {
	...
	public void change_process() {
		...
		try {
			oos.writeObject(Protocol.CHANGE
					...
					+Protocol.seperator
					+nickName+"님의대화명이 "+afterName+"으로 변경되었습니다.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
